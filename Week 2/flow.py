...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        """
        Transform from Base Distribution (z) -> Data (x)
        This corresponds to the Generative direction.
        Equation: x = z_frozen + (z_active * exp(s) + t)
        """
        ...
        # The networks take the entire vector, but we only care about the dependency on z_frozen.
        # Masking the input to the networks (z_frozen) ensures s and t only depend on the frozen part.
        s = self.scale_net(z_frozen)
        t = self.translation_net(z_frozen)

        # 3. Apply the Affine Transformation to the active part
        # We multiply s and t by (1-mask) to ensure we don't modify the frozen dimensions
        x = z_frozen + (1 - self.mask) * (z_active * torch.exp(s) + t)

        # 4. Calculate Log Determinant
        # The Jacobian is triangular. The determinant is the product of the diagonal elements.
        # For frozen dims, derivative is 1 (log(1)=0).
        # For active dims, derivative is exp(s) (log(exp(s))=s).
    ...
    def inverse(self, x):
        """
        Transform from Data (x) -> Base Distribution (z)
        This corresponds to the Normalizing/Inference direction used for training.
        Equation: z = x_frozen + (x_active - t) * exp(-s)
        """
        ...
        # 2. Compute s and t
        # Crucial: Because x_frozen == z_frozen (from forward), s and t are identical 
        # to what was computed in the forward pass.
        s = self.scale_net(x_frozen)
        t = self.translation_net(x_frozen)

        # 3. Apply Inverse Affine Transformation
        # We invert the forward operation: z = (x - t) / exp(s) <=> (x - t) * exp(-s)
        z = x_frozen + (1 - self.mask) * (x_active - t) * torch.exp(-s)

        # 4. Calculate Log Determinant of the Inverse
        # The derivative of the inverse function is exp(-s).
        # Log det = sum(-s)
        log_det_J = -torch.sum((1 - self.mask) * s, dim=1)
...
if __name__ == "__main__":
    ...
    # Increase complexity
    num_transformations = 8   # Increased from 5
    num_hidden = 64           # Increased from 8 (Crucial for learning the curve)
    ...
    for i in range(num_transformations):
        ...
        # Deeper/Wider networks for scale and translation
        scale_net = nn.Sequential(
            nn.Linear(D, num_hidden), 
            nn.Tanh(),                 # Tanh often works better than ReLU for flows (stability)
        ...
        translation_net = nn.Sequential(
            nn.Linear(D, num_hidden), 
            nn.Tanh(),                 # Tanh often works better than ReLU for flows
            nn.Linear(num_hidden, num_hidden), 
            nn.Tanh(), 
            nn.Linear(num_hidden, D)
        )
