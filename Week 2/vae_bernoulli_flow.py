...
if __name__ == "__main__":
    ...
    # Load Data (Binarized for Bernoulli VAE)
    print("Loading Binarized MNIST...")
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Lambda(lambda x: (x > 0.5).float()), # Binarize [0,1] -> {0,1}
    ...
    if args.mode == 'train':
        ...
        # 3. Generate Samples
        with torch.no_grad():
            samples = model.sample(64).cpu().view(64, 1, 28, 28)
